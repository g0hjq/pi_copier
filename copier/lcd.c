...
static void lcd_display_message_no_lock(const char *line0, const char *line1, const char *line2, const char *line3)
{
    char buf[21];
...
// This used to talk to the LCD without holding lcd_mutex. It is called from the GPIO
// thread (via display_system_menu) as well as the main loop, so its 4-bit nibbles could
// interleave with lcd_write_string()/lcd_display_bargraph() and desync the HD44780.
void lcd_display_message(const char *line0, const char *line1, const char *line2, const char *line3)
{
	pthread_mutex_lock(&lcd_mutex);
	lcd_display_message_no_lock(line0, line1, line2, line3);
	pthread_mutex_unlock(&lcd_mutex);
}
